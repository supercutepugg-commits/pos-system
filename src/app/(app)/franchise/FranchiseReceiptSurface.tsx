  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-5 p-6">
      ...
      <div className="border-border bg-card flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border">
        <div className="min-h-0 flex-1 overflow-auto rounded-t-xl"><table className="w-full min-w-[1500px] border-collapse text-[12.5px]"><thead><tr className="bg-surface-subtle border-border sticky top-0 z-10 border-b"><th className="w-10 px-3 py-2.5 text-left"><input aria-label="전체 선택" type="checkbox" checked={props.allChecked} onChange={props.onToggleAll} className="accent-primary size-[15px] cursor-pointer" /></th>{['접수일','접수 채널','사업자 유형','상호명','대표자','연락처','등록자','담당자','인터넷','상태','진행률','메모'].map(label => <th key={label} className="text-muted-foreground px-2.5 py-2.5 text-left font-semibold whitespace-nowrap">{label}</th>)}</tr></thead>
          <tbody>
            {props.rows.length === 0 && <tr className="border-border border-b"><td colSpan={13} style={{ height: 50 * 49 }} className="text-muted-foreground text-center text-sm">조건에 맞는 접수 건이 없습니다.</td></tr>}
            ...
            {props.rows.length > 0 && Array.from({ length: Math.max(0, 50 - props.rows.length) }).map((_, index) => <tr key={`filler-${index}`} aria-hidden="true" className="border-border border-b"><td colSpan={13} style={{ height: 49 }} /></tr>)}
          </tbody></table></div>

  ...
  return (
    <FormModal title="고객 관리 대장 등록" onClose={onClose} maxWidthClassName="max-w-lg">
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <button type="submit" disabled={submitting || !form.business_name.trim()}
...
const EditableField = memo(function EditableField({ row, field, onSave, type = 'text', format, required }: EditableFieldProps) {
  const initial = (row[field] as string) ?? ''
  const [value, setValue] = useState(initial)

  function commit() {
    const trimmed = value.trim()
    if (required && !trimmed) { setValue(initial); return }
...
export default function CustomerLedgerClient({ rows, profile }: Props) {
  ...
  const filteredRows = useMemo(() => {
    ...
    return localRows.filter(row => {
      const haystack = `${row.business_name} ${row.phone ?? ''} ${row.manager_name ?? ''}`.toLowerCase()
  ...
  const saveField = useCallback(async (row: CustomerLedger, field: keyof CustomerLedger, value: string) => {
    const supabase = createClient()
    const { error } = await supabase.from('customer_ledger').update({ [field]: value || null }).eq('id', row.id)
    if (error) { toast.error('수정 실패: ' + error.message); return }
    setLocalRows(prev => prev.map(r => r.id === row.id ? { ...r, [field]: value || null } : r))
  }, [toast])
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {showForm && (
        <CreateForm onSubmit={handleCreate} submitting={submitting} onClose={() => setShowForm(false)} />
      )}
        ...
        <table className="w-full text-sm border-collapse">
          <thead className="bg-slate-50 sticky top-0 z-10">
            <tr>
              ...
              <th className="text-left px-3 py-3 font-semibold text-slate-700 border-b border-slate-200 whitespace-nowrap">날짜</th>
              <th className="text-left px-3 py-3 font-semibold text-slate-700 border-b border-slate-200 whitespace-nowrap">상호명</th>
              <th className="text-left px-3 py-3 font-semibold text-slate-700 border-b border-slate-200 whitespace-nowrap">연락처</th>
              <th className="text-left px-3 py-3 font-semibold text-slate-700 border-b border-slate-200 whitespace-nowrap">담당자</th>
              <th className="text-left px-3 py-3 font-semibold text-slate-700 border-b border-slate-200">문제상황</th>
              <th className="text-left px-3 py-3 font-semibold text-slate-700 border-b border-slate-200">해결방안</th>
            </tr>
          </thead>
          <tbody>
            {filteredRows.map(row => (
              <Fragment key={row.id}>
                <tr className="border-b border-slate-100 hover:bg-blue-50 transition-colors cursor-pointer" onClick={() => toggleExpand(row.id)}>
                  ...
                  <td className="px-3 py-3 font-medium text-slate-900 whitespace-nowrap min-w-[140px]">{row.business_name}</td>
                  <td className="px-3 py-3 text-slate-700 whitespace-nowrap">{row.phone ?? '-'}</td>
                  <td className="px-3 py-3 text-slate-700 whitespace-nowrap">{row.manager_name ?? '-'}</td>
                  <td className="px-3 py-3 text-slate-700 max-w-[220px] truncate">{row.issue ?? '-'}</td>
                  <td className="px-3 py-3 text-slate-700 max-w-[220px] truncate">{row.solution ?? '-'}</td>
                </tr>
                {expandedId === row.id && (
                  <tr className="bg-blue-50/50 border-b border-slate-100" onClick={e => e.stopPropagation()}>
                    <td colSpan={7} className="px-6 py-4">
                      <div className="grid grid-cols-4 gap-4">
                        <div>
                          <label className="text-xs font-semibold text-slate-400">날짜</label>
                          <EditableField row={row} field="record_date" type="date" required onSave={saveField} />
                        </div>
                        <div>
                          <label className="text-xs font-semibold text-slate-400">상호명</label>
                          <EditableField row={row} field="business_name" required onSave={saveField} />
                        </div>
                        <div>
                          <label className="text-xs font-semibold text-slate-400">연락처</label>
                          <EditableField row={row} field="phone" format={formatPhone} onSave={saveField} />
                        </div>
                        <div>
                        ...
                        <div className="col-span-4">
                          ...
                          <EditableField row={row} field="issue" type="textarea" onSave={saveField} />
                        </div>
                        <div className="col-span-4">
                          <label className="text-xs font-semibold text-slate-400">해결방안</label>
                          <EditableField row={row} field="solution" type="textarea" onSave={saveField} />
                        </div>
                      </div>
                    </td>
                  </tr>

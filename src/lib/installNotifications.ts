...
function notificationStatus(deliveryType: string | null, status: string) {
  if (deliveryType === 'delivery' && (status === 'preparing' || status === 'completed')) return null
  if (deliveryType === 'as' && status !== 'in_transit') return null

  const value = deliveryType === 'delivery' && status === 'in_transit' ? 'delivery_sent' : status
  return ['preparing', 'scheduled', 'in_transit', 'completed', 'delivery_sent'].includes(value) ? value : null
}
...
export async function sendApprovedInstallNotification({
  ...
  const { data, error: lookupError } = await admin
    .from('installations')
    .select('id, status, customer_name, customer_phone, delivery_type, status_token, scheduled_date, scheduled_time, tracking_number')
  ...
  if (installation.status !== status) return { error: '현재 설치 상태와 알림톡 발송 상태가 다릅니다.' }
  ...
  try {
    ...
    if (logError) return { error: `알림톡은 발송됐지만 감사 로그 저장에 실패했습니다: ${logError.message}` }

    const { error: notifyStateError } = await admin
      .from('installations')
      .update({ last_notify_status: templateKey, last_notify_at: sentAt })
      .eq('id', installationId)
    if (notifyStateError) return { error: `발송 로그는 저장됐지만 최종 발송 상태 갱신에 실패했습니다: ${notifyStateError.message}` }
    return { error: null }
  } catch (error) {
    ...
    return { error: failureLogError ? `${message} (실패 로그 저장 오류: ${failureLogError.message})` : message }
  }
}

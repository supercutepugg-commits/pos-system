...
const ROLE_COLOR: Record<string, string> = {
  admin: 'text-purple-600', sales: 'text-blue-600', cs: 'text-emerald-600', tech: 'text-orange-600',
}
...
export default function ChatRoom({ profile, initialMessages }: { profile: Profile; initialMessages: Message[] }) {
  ...
  useEffect(() => {
    const channel = supabase.channel('global-chat')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, async (payload) => {
        const { data: msg } = await supabase.from('messages').select('*, user:profiles(id, name, role)').eq('id', payload.new.id).single()
  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    setSending(true)
    await supabase.from('messages').insert({ user_id: profile.id, content: input.trim() })
    setInput('')
    setSending(false)
  }

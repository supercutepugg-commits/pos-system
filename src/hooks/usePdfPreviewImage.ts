...
export function usePdfPreviewImage(imageUrl: string | null): Result {
  ...
  const [error, setError] = useState<string | null>(imageUrl ? null : '계약서 이미지를 생성하지 못했습니다.')
  ...
  useEffect(() => {
    ...
    function handleError() { setError('계약서 이미지를 불러오지 못했습니다.') }
  ...
  return { imgRef, renderedWidth: size?.width ?? null, renderedHeight: size?.height ?? null, error }
}

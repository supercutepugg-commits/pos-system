...
function parseEquipment(product) {
  if (!product) return []
  // Split by comma or +
  const parts = product.split(/[,+]/).map(s => s.trim()).filter(Boolean)
  const catalog = ['토스프론트', '포스기', '인터넷', '키오스크', '영수증프린터', '키오스크리더기', '무선단말기', '금전함', '태블릿', '테이블오더', '이동식단말기', '무선프린트', '원격']
  const items = []
...
function normalizeInternet(v) {
  ...
  if (s.includes('백매가') || s.includes('백메가') || s.includes('백M') || s.includes('백m')) return '백메가'
  if (s === '') return null
  return s
}
...
const rows = dataRows
  .map((r, i) => {
    ...
    return {
      ...
      equipment: equipmentItems.length > 0 ? equipmentItems.map(i => `${i.name}×${i.quantity}`).join(', ') : clean(r[15]),

...
import { FRANCHISE_STATUS_LABEL, STATUS_LABEL, type FranchiseStatus, type TicketStatus } from '@/types'
...
function statusLabel(source: EmployeeActivityLog['source'], status: string | null) {
  ...
  if (source === 'franchise') {
    ...
    return FRANCHISE_ACTIVITY_LABEL[status]
...
export default function LogsClient({
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter(log => {
      ...
      if (!q) return true
      return log.actorName.toLowerCase().includes(q)
    ...
    <>
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <input
          type="date"
          value={selectedDate ?? ''}
          onChange={event => event.target.value && router.push(`/admin/logs?date=${event.target.value}`)}
          className="rounded-lg border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
        <button onClick={() => router.push(`/admin/logs?date=${todayStr()}`)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-600 hover:bg-slate-50">
          오늘
        </button>
        {selectedDate && (
      ...
      <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
        <SourceButton label="전체" count={logs.length} active={source === 'all'} onClick={() => setSource('all')} />
        {(['franchise', 'installation', 'ticket', 'inventory'] as const).map(item => (
          <SourceButton
            key={item}
            label={{ franchise: '가맹접수', installation: '설치', ticket: '작업', inventory: '재고' }[item]}
      ...
      {userCounts.length > 0 && (
        <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="mb-2 text-xs font-semibold text-slate-500">{selectedDate ? `${selectedDate} 담당자별 처리 건수` : '담당자별 처리 건수'}</p>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="divide-y divide-slate-50">
          {filtered.map(log => {
            const deliveryType = typeof log.details?.delivery_type === 'string' ? log.details.delivery_type : null
            return (
              <div key={log.id} className="px-5 py-3.5">
                <div className="flex flex-wrap items-center gap-1.5 text-xs text-slate-600">
                  <span className={`rounded-full px-2 py-0.5 font-semibold ${SOURCE_TONE[log.source]}`}>{log.sourceLabel}</span>
                ...
                {(log.fromStatus || log.toStatus) && (
                  <div className="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
                    ...
                    <ArrowRight size={11} />
                    <span className="font-medium text-slate-700">{statusLabel(log.source, log.toStatus)}</span>
                    {deliveryType && isInstallationDeliveryType(deliveryType) && (
                      <span className="rounded-full bg-blue-50 px-2 py-0.5 font-semibold text-blue-700">구분: {INSTALLATION_DELIVERY_TYPE_LABEL[deliveryType]}</span>
                    )}
                  </div>
                )}
                {log.description && (
                  <p className="mt-1 text-xs text-slate-600">
                    {log.source === 'installation' ? INSTALLATION_ACTION_LABEL[log.description] ?? log.description : log.description}
                  </p>
                )}
                <p className="mt-1 text-xs text-slate-400">{format(new Date(log.createdAt), 'yyyy-MM-dd HH:mm', { locale: ko })}</p>
              </div>
            )
          })}

...
export default function ApprovalNoteTimeline({ notes }: { notes: ApprovalNote[] }) {
  ...
  return (
    <ol className="space-y-3">
      {[...notes].sort((a, b) => a.created_at.localeCompare(b.created_at)).map(note => (
        <li key={note.id} className="border-l-2 border-blue-200 pl-3">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs">
            ...
            <span className="text-slate-500">{ROLE_LABEL[note.author_role] ?? note.author_role}</span>
            <span className={note.stage === 'rejection' ? 'font-medium text-red-600' : 'font-medium text-blue-600'}>
              {STAGE_LABEL[note.stage]}
            </span>
            <time className="text-slate-400" dateTime={note.created_at}>{formatTimestamp(note.created_at)}</time>
          </div>
          <p className="mt-1 whitespace-pre-wrap break-words text-sm text-slate-700">{note.content}</p>
        </li>
      ))}
    </ol>
  )

...
export default function NameEdit({ userId, initialName }: Props) {
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      if (error) { toast.error('이름 변경 실패: ' + error); setDraft(name); return }
      setName(next)
      setEditing(false)
    })
  }
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      />
      <button onClick={save} disabled={isPending} className="text-emerald-500 hover:text-emerald-600 disabled:opacity-50">
        <Check size={14} />
      </button>
      <button onClick={() => { setEditing(false); setDraft(name) }} disabled={isPending} className="text-slate-300 hover:text-slate-500 disabled:opacity-50">
        <X size={14} />
      </button>
    </div>
  )

...
export default function GroupChatRoom({ profile, room, initialMessages }: Props) {
  ...
  async function sendMessage(event: React.FormEvent) {
    ...
    const { error } = await supabase
      .from("group_chat_messages")
      .insert({ room_id: room.id, user_id: profile.id, content });
    ...
    return {
      ...
      showDate:
        ...
        format(new Date(message.created_at), "yyyy-MM-dd") !==
          format(new Date(previous.created_at), "yyyy-MM-dd"),
      showProfile:
        !previous ||
  ...
  return (
    <div className="flex h-screen flex-col bg-[#b2c7d9]">
              ...
              {message.showDate && (
                <div className="my-4 flex justify-center">
                  <span className="rounded-full bg-black/20 px-3 py-1 text-xs text-white">
                    {format(new Date(message.created_at), "yyyy년 M월 d일 (EEE)", { locale: ko })}
                  </span>
                </div>
              )}
              <div className={`flex items-end gap-2 ${isMe ? "flex-row-reverse" : ""}`}>
                {!isMe && (
                  <div className="w-9 self-start">
                    {message.showProfile && (
                      <div className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-sm font-bold text-slate-700">
                        {message.user?.name?.[0] ?? "?"}
                      </div>
                    )}
                  </div>
                ...
                <div className={`flex max-w-[70%] flex-col ${isMe ? "items-end" : "items-start"}`}>
                  {!isMe && message.showProfile && (
                    <span className="mb-1 ml-1 text-sm font-bold text-slate-800">
                      {message.user?.name}
                    </span>
                  )}
                  <div className={`flex items-end gap-1.5 ${isMe ? "flex-row-reverse" : ""}`}>
                    <div
                      className={`break-words rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed shadow-sm ${isMe ? "rounded-tr-sm bg-[#fee500] text-slate-900" : "rounded-tl-sm bg-white text-slate-900"}`}
                    >
                      {message.content}
                    </div>
                    <span className="mb-0.5 whitespace-nowrap text-[10px] text-white/70">
      ...
      <form
        ...
        className="flex shrink-0 items-center gap-2 border-t border-slate-200 bg-white px-3 py-3"
      >
        <input
          value={input}
          maxLength={4000}
          ...
          className="flex-1 rounded-full bg-slate-100 px-4 py-2.5 text-sm text-slate-900 placeholder-slate-400 outline-none focus:ring-2 focus:ring-blue-500"
        />
        <button
          type="submit"
          disabled={!input.trim() || sending}
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#fee500] shadow-sm disabled:opacity-40"
        >
